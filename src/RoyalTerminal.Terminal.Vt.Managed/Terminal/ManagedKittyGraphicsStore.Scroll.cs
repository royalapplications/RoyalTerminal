...
internal sealed partial class ManagedKittyGraphicsStore
{
        ...
        uint cellWidth, uint cellHeight)
    {
        ...
        foreach ((PlacementKey key, Placement placement) in _placements)
        {
            ...
            // This integration moves rows in place, so pins outside the region
            // (including history) need no restoration or geometry work.
            ...
            if (delta != 0 && _images.TryGetValue(key.ImageId, out Image? image))
            {
                ...
                if (grid.Rows > 0 && grid.Columns > 0 && (long)row + grid.Rows - 1 <= bottom)
                {
