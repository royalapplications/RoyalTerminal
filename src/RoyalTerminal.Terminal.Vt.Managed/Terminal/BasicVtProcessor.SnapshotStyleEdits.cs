...
public sealed partial class BasicVtProcessor
{
    ...
    {
        using GhosttySnapshotStyleTracker.RowEdit styles = _screen.EditSnapshotRowStyles(row);
        if (_screen.TracksSnapshotStyles) styles.Write(column, CaptureSnapshotPen());
    ...
    {
        using GhosttySnapshotStyleTracker.RowEdit styles = _screen.EditSnapshotRowStyles(row);
        if (_screen.TracksSnapshotStyles) styles.Write(column, GhosttySnapshotLivePage.EncodeStyle(in cell));
        row[column] = cell;
    }
}
